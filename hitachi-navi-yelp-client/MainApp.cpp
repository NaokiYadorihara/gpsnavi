...
int MainApp::AuthenticatePOI(const QString & CredentialsFile)
{
    ...
    if (fgets(buf, 5125, filep))
    {
	    if (strlen(buf) > 0 && buf[strlen(buf)-1] == '\n')
			buf[strlen(buf)-1] = '\0';
		ProxyHost = QString(buf);
		ProxyHost.replace(0, 10, QString(""));
    }
    ...
    if (fgets(buf, 5125, filep))
    {
	    if (strlen(buf) > 0 && buf[strlen(buf)-1] == '\n')
			buf[strlen(buf)-1] = '\0';
		ProxyPort = QString(buf);
		ProxyPort.replace(0, 10, QString(""));
    }
    ...
    if (fgets(buf, 5125, filep))
    {
	    if (strlen(buf) > 0 && buf[strlen(buf)-1] == '\n')
			buf[strlen(buf)-1] = '\0';
		ProxyUser = QString(buf);
		ProxyUser.replace(0, 10, QString(""));
    }
    ...
    if (fgets(buf, 5125, filep))
    {
	    if (strlen(buf) > 0 && buf[strlen(buf)-1] == '\n')
			buf[strlen(buf)-1] = '\0';
		ProxyPassword = QString(buf);
		ProxyPassword.replace(0, 14, QString(""));
    }
	...
	if( (true != ProxyHost.isEmpty()) &&
		(true != ProxyPort.isEmpty()) &&
		(true != ProxyUser.isEmpty()) &&
		(true != ProxyPassword.isEmpty()) )
	{
		proxy.setType(QNetworkProxy::Socks5Proxy);
		proxy.setType(QNetworkProxy::HttpProxy);
		proxy.setHostName(ProxyHost);
		proxy.setPort(ProxyPort.toInt());
		proxy.setUser(qPrintable(ProxyUser));
